...
{
    public sealed class LiveChartsSelectionViewModel : ToolPaneViewModel
    {
        ...
        private readonly Dictionary<string, ObservableCollection<ISeries>> _plotModelsDict = new Dictionary<string, ObservableCollection<ISeries>>();
        ...
        */
        ...
        private Task SetAndProcessPlot(PlotSerieTypes serieTypes, TimeSpan period, CancellationToken cancelationToken)
        {
            return Task.Run(() =>
            {
                ...
                foreach (var serie in SelectedSeries)
                {
                    /*
        ...
        public LiveChartsSelectionViewModel(IMessenger messenger, ISettingsManager settingsManager, ILogger<LiveChartsSelectionViewModel> logger)
            : base(messenger, settingsManager, logger)
        {
            ...
            Messenger.Register<LiveChartsSelectionViewModel, SessionUpdateMessage>(this, (r, m) =>
            {
            ...
            Messenger.Register<LiveChartsSelectionViewModel, SessionClosedMessage>(this, (r, _) => r.Clear());
            Messenger.Register<LiveChartsSelectionViewModel, TradeSelectedMessage>(this, (r, m) => r.ProcessTradeSelected(m));
            ...
            _resultBgWorker.ProgressChanged += (s, e) =>
            {
                ...
                {
                    case 0:
                        PlotModels.Add((ObservableCollection<ISeries>)e.UserState);
        ...
        private ObservableCollection<ObservableCollection<ISeries>> _plotModels = new ObservableCollection<ObservableCollection<ISeries>>();
        public ObservableCollection<ObservableCollection<ISeries>> PlotModels
        {
        ...
        private ObservableCollection<ISeries> _selectedSeries;
        public ObservableCollection<ISeries> SelectedSeries
        {
        ...
        private void ParseResult(Result result)
        {
            foreach (var chart in result.Charts.OrderBy(x => x.Key))
            {
                ...
                {
                    // Create Plot
                    plot = new ObservableCollection<ISeries>();
                ...
                foreach (var serie in chart.Value.Series.OrderBy(x => x.Key))
                {
                    ...
                    var s = plot.FirstOrDefault(k => k.Name == serie.Value.Name);
                    ...
                    if (s == null)
                    {
                        ...
                        switch (serie.Value.SeriesType)
                        {
                            ...
                            case SeriesType.Candle:
                                s = new CandlesticksSeries<FinancialPoint>
                                {
                                    Name = serie.Value.Name,
                                };
                                plot.Add(s);
                                /*
                            ...
                            case SeriesType.Line:
                                s = new LineSeries<DateTimePoint>
                                {
                                    Name = serie.Value.Name,
...
#pragma warning restore RCS1079 // Throwing of new NotImplementedException.
                    ...
                    switch (serie.Value.SeriesType)
                    {
                        ...
                        case SeriesType.Line:
                            serie.Value.Values.ForEach(p => ((ObservableCollection<DateTimePoint>)((LineSeries<DateTimePoint>)s).Values).Add(new DateTimePoint(p.X.UtcDateTime, (double)p.Y)));
                            /*
        ...
        */
        ...
        private void AddPlotThreadUI(ObservableCollection<ISeries> plot)
        {
    ...
    /*
    internal sealed class ScatterPointComparer : IEqualityComparer<ObservablePoint>
    {
        public bool Equals(ObservablePoint p1, ObservablePoint p2)
        {
            return p1.X == p2.X && p1.Y == p2.Y;// && p1.Size == p2.Size;
        }

        public int GetHashCode([DisallowNull] ObservablePoint pt)
        {
            return (pt.X, pt.Y).GetHashCode(); // , pt.Size
        }
    }
    */
}
